...
export const TopBar: React.FC = () => {
  ...
  return (
    <header className="h-[56px] min-h-[56px] px-3 sm:px-4 bg-[#07111e]/95 backdrop-blur-md border-b border-white/10 flex items-center justify-between z-30 select-none">
      ...
      {/* Right: Sound + Admin CRM + GDD Concept + Settings */}
      <div className="flex items-center gap-1 sm:gap-1.5">
        ...
        {/* ADMIN CRM QUICK ACCESS BUTTON */}
        <button
          onClick={() => setCurrentScreen('admin')}
          className="flex items-center gap-1 bg-gradient-to-r from-red-500/20 via-purple-500/20 to-cyan-500/20 border border-red-400/60 text-red-300 hover:text-white px-2 py-1 rounded-lg hover:border-red-300 transition-all text-[11px] font-black tracking-wider shadow-sm"
          title="Панель Администратора, LiveOps & CRM"
        >
        ...
        <button
          ...
          className="flex items-center gap-1 bg-gradient-to-r from-cyan-500/20 to-purple-500/20 border border-cyan-400/40 text-cyan-200 px-2 py-1 rounded-lg hover:border-cyan-300 transition-colors text-[11px] font-bold tracking-wider"
