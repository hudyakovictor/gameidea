...
export const BattlerScreen: React.FC = () => {
  ...
  const [combatLog, setCombatLog] = useState<string[]>([
    'Рынок вошел в фазу аномальной волатильности. Леверидж-Гоблин готовит сокрушительную атаку!',
  ]);
  ...
  const handleExecuteTurn = () => {
    ...
    playedCards.forEach((card) => {
      ...
      if (card.category === boss.weaknessCategory) {
        hasCounter = true;
        totalDamage += 20; // Critical counter bonus
      }
    });
  ...
  return (
    <div className="h-full flex flex-col justify-between overflow-hidden p-2 sm:p-3 max-w-5xl mx-auto w-full select-none">
      ...
      <div className="bg-[#091522] border border-red-500/30 rounded-xl p-2.5 sm:p-3 shadow-xl">
        <div className="flex items-center justify-between gap-2">
          {/* Boss Info + Portrait */}
          <div className="flex items-center gap-2.5">
            <div className="relative w-12 h-12 sm:w-14 sm:h-14 rounded-xl overflow-hidden border-2 border-red-500/60 shadow-lg shadow-red-950/50 flex-shrink-0">
              <img
                src={boss.image}
                alt={boss.nameRu}
        ...
        {/* Boss Intent Badge */}
        <div className="mt-2 pt-2 border-t border-white/10 flex items-center justify-between text-[11px] bg-red-950/30 px-2.5 py-1 rounded-lg">
          ...
          <span className="text-[10px] text-slate-400 font-mono">
            Уязвимость: <strong className="text-cyan-300 uppercase">{boss.weaknessCategory}</strong>
          </span>
        </div>
      </div>

      {/* Middle Section: Market Anomaly Scenario Visualizer */}
      <div className="my-1.5 bg-[#050e18] border border-cyan-500/20 rounded-xl p-2.5 flex-1 flex flex-col justify-between overflow-hidden shadow-inner relative">
        {/* Background Candlestick Pattern Silhouette */}
        <div className="absolute inset-0 opacity-10 flex items-end justify-between px-4 pb-2 pointer-events-none">
          {[40, 65, 30, 85, 45, 95, 60, 110, 75, 50, 90, 40].map((h, i) => (
            <div key={i} className="flex flex-col items-center">
              <div className="w-0.5 bg-cyan-400" style={{ height: h + 15 }} />
              <div
                className={`w-3 ${i % 2 === 0 ? 'bg-emerald-400' : 'bg-red-500'}`}
                style={{ height: h }}
              />
              <div className="w-0.5 bg-cyan-400" style={{ height: h / 2 }} />
            </div>
          ))}
        </div>
        ...
        <div className="relative z-10 space-y-1">
          <div className="text-[10px] font-mono text-cyan-300 uppercase tracking-wider flex items-center gap-1">
            <Sparkles size={11} /> Сценарий: Инфляция и ликвидация плеч
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-1 text-[10px] text-slate-300">
            <span className="bg-black/50 border border-white/5 px-2 py-0.5 rounded flex items-center gap-1">
              ⚡ Слом структуры тренда (4H)
            </span>
            <span className="bg-black/50 border border-white/5 px-2 py-0.5 rounded flex items-center gap-1 text-red-300">
              ⚠️ Лавинообразный объем продаж
            </span>
            <span className="bg-black/50 border border-white/5 px-2 py-0.5 rounded flex items-center gap-1 text-amber-300">
        ...
        {/* Combat Action Log */}
        <div className="relative z-10 my-1 bg-black/60 border border-white/10 rounded-lg p-2 text-[10px] sm:text-xs font-mono text-slate-300 leading-relaxed max-h-16 overflow-y-auto">
      ...
      <div className="bg-[#071320] border border-white/10 rounded-xl p-2 sm:p-2.5">
        ...
        <div className="mt-2 flex items-center gap-2">
          ...
          >
            ...
            <span>Применить карты протокола ({selectedCardIds.length})</span>
          </button>
        </div>
      </div>
    </div>
